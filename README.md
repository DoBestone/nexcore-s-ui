...
- 订阅端口:`2096`
- 订阅路径:`/sub/`
- 管理员凭据:首装由 `install.sh` **自动随机生成并打印**(立即记录;之后只能 `s-ui` 菜单重置)
...
`update.sh` 只替换 sui 二进制 + `bin/` + service unit,**`/usr/local/s-ui/db/` 永远不动**(数据库完整保留)。
...
TOKEN="<your-token>"
HOST="http://your-server:2095/app"
...
默认监听 `2095`,浏览器打开 `http://127.0.0.1:2095/app/`。开发期临时账号通过 CLI 创建:
...
> 注:`/usr/local/s-ui/db/` 是数据库目录(订阅、客户端、入站、TLS 等),`rm -fr /usr/local/s-ui` 会一并清掉。如需保留请先单独备份。
