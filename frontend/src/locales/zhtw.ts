export default {
  ...
  api: {
    desc: "面板的全部能力都通過 API 暴露,使用 Bearer Token 直接接入第三方系統。",
    tab: { tokens: "Token 管理", docs: "API 文件", logs: "呼叫日誌" },
    tokens: {
      search: "搜尋 desc 或 token…",
      create: "新建 Token",
      created: "Token 已建立,請立即記錄 — 關閉頁面後無法回查。",
      desc: "備註",
      descHint: "例:CI 部署機 / 運維腳本",
      token: "Token",
      expiry: "有效期",
      expiryHint: "0 表示永久;其它整數代表多少天後過期。",
      empty: "尚未建立任何 Token",
    },
    docs: {
      title: "API 端點速查",
      hintBearer: "所有 /apiv2/* 呼叫必須帶 Authorization: Bearer <TOKEN>(或 Token: <TOKEN> 標頭)。",
      search: "依方法或路徑過濾…",
      group: { auth: "驗證", data: "資料查詢", mut: "變更操作", tokens: "Token 自管理", audit: "稽核 / 日誌" },
      col: { method: "方法", path: "路徑", desc: "說明" },
      me: { desc: "驗證 Token,返回當前 username 與 token desc。" },
      load: { desc: "全量載入面板資料。" },
      inbounds: { desc: "查詢入站。" },
      outbounds: { desc: "查詢出站。" },
      endpoints: { desc: "查詢端點。" },
      services: { desc: "查詢服務。" },
      tls: { desc: "列出所有 TLS 設定。" },
      clients: { desc: "查詢客戶端。" },
      config: { desc: "原始 sing-box 設定 JSON。" },
      users: { desc: "管理員清單。" },
      settings: { desc: "面板設定。" },
      stats: { desc: "流量統計。" },
      status: { desc: "系統狀態。" },
      onlines: { desc: "即時在線連線。" },
      logs: { desc: "sing-box 執行日誌。" },
      changes: { desc: "面板變更歷史。" },
      keypairs: { desc: "產生 keypair。" },
      getdb: { desc: "匯出 sqlite 資料庫。" },
      checkOutbound: { desc: "檢測出站可達性。" },
      singboxConfig: { desc: "下載 sing-box 設定。" },
      save: { desc: "通用儲存入口。" },
      restartApp: { desc: "重啟面板。" },
      restartSb: { desc: "重啟 sing-box。" },
      linkConvert: { desc: "解析訂閱連結。" },
      subConvert: { desc: "解析訂閱 URL。" },
      importdb: { desc: "上傳 sqlite 覆寫 DB。" },
      changePass: { desc: "修改管理員帳號或密碼。" },
      setting: { desc: "更新面板/訂閱 port 與 path。" },
      tokens: { desc: "查詢當前用戶的 token。" },
      addToken: { desc: "建立新 Token。" },
      deleteToken: { desc: "撤銷 Token。" },
      apiLogs: { desc: "稽核日誌。" },
      clearApiLogs: { desc: "清空 API 稽核日誌。" },
    },
    logs: {
      method: "方法", path: "路徑", username: "使用者", time: "時間", status: "狀態",
      latency: "耗時", tokenDesc: "Token 備註", ip: "來源 IP", err: "錯誤",
      clear: "清空日誌",
      clearConfirm: "確定清空所有 API 呼叫日誌?此操作無法復原。",
    },
  },
  main: {
    tiles: "信息卡",
